...
// Labeling API
export const getLabelingDatasets = async () => {
  const response = await fetch(`${BASE_URL}/api/labeling/datasets`, {
    headers: getAuthHeaders(),
  });
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to fetch labeling datasets");
  }
  return response.json();
};

export const createLabelingDataset = async (data: { name: string; task: string; target_labels?: string[] }) => {
  const response = await fetch(`${BASE_URL}/api/labeling/datasets`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: JSON.stringify(data),
  ...
  if (!response.ok) {
    ...
    throw new Error(error.detail || "Failed to create labeling dataset");
  }
  return response.json();
};

export const getLabelingDataset = async (datasetId: string) => {
  const response = await fetch(`${BASE_URL}/api/labeling/datasets/${datasetId}`, {
    headers: getAuthHeaders(),
  });
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to fetch labeling dataset");
  }
  return response.json();
};

export const updateLabelingDataset = async (datasetId: string, data: { name?: string; target_labels?: string[] }) => {
  const response = await fetch(`${BASE_URL}/api/labeling/datasets/${datasetId}`, {
    method: "PATCH",
    headers: getAuthHeaders(),
    body: JSON.stringify(data),
  ...
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to update labeling dataset");
  }
  return response.json();
};

export const deleteLabelingDataset = async (datasetId: string) => {
  const response = await fetch(`${BASE_URL}/api/labeling/datasets/${datasetId}`, {
    method: "DELETE",
    headers: getAuthHeaders(),
  });
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to delete labeling dataset");
  }
};

export const uploadLabelingFiles = async (datasetId: string, files: File[]) => {
  ...
  const token = localStorage.getItem("token");
  const response = await fetch(`${BASE_URL}/api/labeling/datasets/${datasetId}/files`, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${token}`,
    },
...
export const analyzeLabelingFiles = async (fileIds: string[]) => {
  const response = await fetch(`${BASE_URL}/api/labeling/analyze`, {
    method: "POST",
    headers: getAuthHeaders(),
...
export const refineLabelingFile = async (fileId: string, verifiedLabels: string[]) => {
  const response = await fetch(`${BASE_URL}/api/labeling/files/${fileId}/refine`, {
    method: "POST",
    headers: getAuthHeaders(),
  ...
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to refine labels");
  }
  return response.json();
};

export const getLabelSuggestions = async (fileIds: string[]) => {
  const response = await fetch(`${BASE_URL}/api/labeling/suggestions`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: JSON.stringify({ file_ids: fileIds }),
  });
  handleUnauthorized(response);
  if (!response.ok) {
    throw new Error("Failed to get label suggestions");
  }
  return response.json();
};

export const exportLabelingDataset = async (datasetId: string, format: "json" | "csv" | "zip") => {
  const token = localStorage.getItem("token");
  const response = await fetch(`${BASE_URL}/api/labeling/datasets/${datasetId}/export?format=${format}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
    },
  });
...
export const deleteLabelingFile = async (fileId: string) => {
  const response = await fetch(`${BASE_URL}/api/labeling/files/${fileId}`, {
    method: "DELETE",
    headers: getAuthHeaders(),
  });
  handleUnauthorized(response);
...
export const getLabelingFile = async (fileId: string) => {
  const response = await fetch(`${BASE_URL}/api/labeling/files/${fileId}`, {
    headers: getAuthHeaders(),
  });
  handleUnauthorized(response);
